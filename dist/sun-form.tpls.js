...
angular.module("sun.form.tpls").run(["$templateCache", function($templateCache) {$templateCache.put("simple-input/simple-input.tpl.html","<div ng-class=\"{ \'has-error\': showError()}\" class=\"form-group\"><label for=\"{{id}}\" class=\"control-label\">{{label  | translate }}</label><div><div inner-content=\"\"></div><div ng-show=\"showError()\" ng-messages=\"inputCtrl.$error\" ng-messages-include=\"defaultFormErrorMessages\" class=\"help-block\"><ng-message ng-repeat=\"message in messages\" when=\"{{message.when}}\">{{message.message}}</ng-message></div><span ng-show=\"helpText\" class=\"help-block\">{{ helpText|translate }}</span></div></div>");
$templateCache.put("form-group/form-group.tpl.html","<label class=\"control-label\">{{label}}</label><div ng-transclude=\"\"></div>");
$templateCache.put("standard-form/standard-form.tpl.html","<form name=\"innerForm\" ng-submit=\"options._form_submit()\"><alert type=\"{{ options.msg.type || \'danger\' }}\" ng-show=\"options.msg.show\" close=\"options.msg.show=false\"><h4 ng-show=\"options.msg.title\">{{options.msg.title | translate}}</h4><span>{{options.msg.text}}</span></alert><ng-transclude></ng-transclude><div ng-show=\"options.buttons\" class=\"btn-group form-buttons pull-right\"><button ng-repeat=\"button in options.buttons\" type=\"{{button.type || \'button\'}}\" ng-class=\"getButtonClass(button)\" ng-click=\"options._clicked(button)\" translate=\"{{button.label}}\"></button></div><div class=\"clearfix\"></div></form>");
$templateCache.put("simple-input/templates/date.tpl.html","<span class=\"input-group\"><input id=\"{{id}}\" name=\"{{name}}\" ng-model=\"ngModel\" type=\"text\" placeholder=\"{{label | translate}}\" datepicker-popup=\"{{format}}\" is-open=\"opened\" class=\"form-control\"/><span class=\"input-group-btn\"><button type=\"button\" ng-click=\"open($event)\" class=\"btn btn-default\"><i class=\"fa fa-calendar\"></i></button></span></span>");
$templateCache.put("simple-input/templates/input.tpl.html","<input id=\"{{id}}\" name=\"{{name}}\" ng-model=\"ngModel\" type=\"{{type}}\" placeholder=\"{{label | translate}}\" class=\"form-control\"/>");
$templateCache.put("simple-input/templates/select.tpl.html","<select for-type=\"select\" id=\"{{id}}\" name=\"{{name}}\" ng-model=\"ngModel\" class=\"form-control\"><option value=\"\" ng-if=\"allowEmpty\">{{emptyLabel}}</option></select>");
$templateCache.put("simple-input/templates/textarea.tpl.html","<textarea for-type=\"textarea\" id=\"{{id}}\" name=\"{{name}}\" ng-model=\"ngModel\" placeholder=\"{{label | translate}}\" class=\"form-control\"></textarea>");}]);